...
**Freezent**는 점차 지능화되고 복잡해지는 주가 조작 및 불공정 거래 행위에 대응하기 위해 개발된 LangGraph 기반 멀티 에이전트 분석 시스템입니다. News_agent, Filings_agent, Lstm_agent 가
각자의 역할을 수행하며 데이터를 수집 및 분석하고, Orchestrator가 이 결과들을 종합하여 최종적으로 투자자에게 분석 리포트를 제공합니다.
**Freezent**는 단순히 과거 주가 데이터를 분석하는 것을 넘어, 공시, 뉴스, 기업 정보, 기술적 지표 등 다양한 비정형+정형 데이터를 실시간으로 분석하여 잠재적인 이상 징후를 포착합니다. 이를 통해 개인 투자자의 합리적인 의사결정을 돕는 것을 목표로 합니다.
...
│   │   └── tools/           (Agent Tools)
│   │   │    └── biz_perf/   (영업(잠정)실적 변화 탐지 도구)
│   │   │    └── corp_info/  (고유번호 반환 도구)
│   │   │    └── floating_stock/  (유동주식 공시검색 도구)
│   │   │    └── lock_up/    (보호예수 기간 검색 도구)
│   │   │    └── lstm_model/ (LSTM-AE 추론 도구)
│   │   │    └── news/       (뉴스루머 분석 도구)
│   │   │    └── paid_in_capital_increase/ (제3자배정유상증자 공시검색도구)
│   │   │    └── stock_info/ (거래량 등 주가 관련 공시검색도구)
...
3.  **DL Model (PyTorch, LSTM-Autoencoder Ensemble)**
    ...
    *   조작 논란이 있는 삼부토건의 1년치 데이터 테스트 결과 플롯입니다. 실제 논란이 있는 5월에서 7월 구간에 비율 값이 높은 것을 볼 수 있습니다.
